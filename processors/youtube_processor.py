...
class YouTubeProcessor:
    ...
    def _get_lyrics(self, title: str, artist: str) -> str:
        ...
        try:
            ...
            # Try multiple combinations for better matching
            search_combinations = [
                (clean_artist, clean_title),
                (clean_title.split(' - ')[0] if ' - ' in clean_title else clean_artist, 
                 clean_title.split(' - ')[1] if ' - ' in clean_title else clean_title)
            ]
            
            for artist_term, title_term in search_combinations:
                # Try Lyrics.ovh API first (free, no key required)
                lyrics = self._get_lyrics_from_ovh(artist_term, title_term)
                if lyrics:
                    return lyrics
            
            # Try web scraping as fallback
            lyrics = self._scrape_lyrics_genius(clean_artist, clean_title)
            if lyrics:
                return lyrics
